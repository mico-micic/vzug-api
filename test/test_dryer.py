...
from vzug import BasicDevice, Dryer, DeviceError
...
def server_hh_default_func():
    ...
    if cmd == const.COMMAND_GET_COMMAND and value == CMD_VALUE_CONSUMP_DRYER_AVG:
        return get_test_response_from_file_raw('dryer_consumption_avg.json')
    elif cmd == const.COMMAND_GET_COMMAND and value == CMD_VALUE_CONSUMP_DRYER_TOTAL:
        return get_test_response_from_file_raw('dryer_consumption_total.json')
    elif cmd == const.COMMAND_GET_MACHINE_TYPE:
        return const.DEVICE_TYPE_SHORT_DRYER
    else:
        return 'WRONG REQUEST'
...
class TestOkAndActiveProgramInformation(LiveServerTestCase, IsolatedAsyncioTestCase):
    ...
    async def test_all_information(self):
        ...
        assert device.device_type is const.DEVICE_TYPE_DRYER
